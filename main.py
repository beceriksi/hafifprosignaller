...
def okx_top_usdt_spot(limit=TOP_N):
    ...
    # daha sağlam filtre: instId ...-USDT
...
def main():
    ...
    for i, inst in enumerate(symbols):
        try:
            ...
            ok_e, ed = early_alert(df1, VOL_MIN_EARLY, VRATIO_EARLY, MOM_1M_MIN)
            if not ok_e:  # erken kıpırdanma hiç yoksa bu coinde oyalanma
                continue
    ...
    # anlık mesaj: sadece sinyal oluştuysa
    lines = []
    if buys:
        buys.sort(key=lambda x: x[0], reverse=True)
        lines += ["📈 *Güvenli BUY Sinyalleri*"] + [m for _, m in buys[:MAX_MSG_COINS]]
    if conts:
        conts.sort(key=lambda x: x[0], reverse=True)
        lines += ["\n⏩ *Momentum Devam Uyarıları*"] + [m for _, m in conts[:MAX_MSG_COINS]]

    if lines:
        head = f"🧭 *OKX Top-{len(symbols)} 1m/5m Tarama*\n⏱ {ts()}\nSELL baskısı (bilgi): {sell_count}"
        telegram(head + "\n" + "\n".join(lines))
    else:
        # sessiz geç (spam yok)
        print(ts(), "— sinyal yok")
